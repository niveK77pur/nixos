{
  devices = {
    optiplex = {
      id = "7PHZLSE-HMGUG2V-BL3JAMO-NGFUTFM-HFKEBSQ-SHAHSAP-RALJWPK-RG65XQS";
      addresses = ["tcp://100.79.7.29:22000"];
    };
    supernote = {
      id = "5O5GDWG-HIR54JQ-SFAZRVF-OIGZ6BJ-R5S56TE-WOBISJL-HTXVB6G-H26A6Q2";
      addresses = ["tcp://100.127.82.92:22000"];
    };
    zenfone = {
      id = "YI45ILA-6JCDX7I-P62UL5I-S4SHZWN-QJ3QPFD-YIG7MM6-K6ENS5O-3WLJUAL";
      addresses = ["tcp://100.109.140.36:22000"];
    };
    tuxedo = {
      id = "QZSWKKG-XWMA7ZF-7DSR6QE-AGM4O2T-TEBBFJI-PKPT63S-MITT4FZ-QGEVTAA";
      addresses = ["tcp://100.74.132.26:22000"];
    };
    titan = {
      id = "2QXPQSO-LHIYXWD-UVXGR6P-KFNEU5E-6SI3YAV-T47GWUI-L72R3D7-TVPRAAO";
      addresses = ["tcp://100.102.201.102:22000"];
    };
  };

  folders = {
    SN-Note-PDF.id = "peedi-tzoyl";
    SN-Note.id = "8uvfz-kien7";
    SN-MyStyle.id = "fpehy-c1mjv";
    VinLudens-Sheets.id = "Vinludens-Sheets";
    Aegis.id = "1b48i-3suyj";
  };
}
